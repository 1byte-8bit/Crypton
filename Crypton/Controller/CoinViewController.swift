...
class CoinViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        cryptoCoins.append(Coins(coinName: "LTC", coinViewName: "Ł LTC"))
        cryptoCoins.append(Coins(coinName: "BTC", coinViewName: "₿ BTC"))
        cryptoCoins.append(Coins(coinName: "DOGE", coinViewName: "Ð DOGE"))
        coins.append(Coins(coinName: "USD", coinViewName: "$ USD"))
        coins.append(Coins(coinName: "RUB", coinViewName: "₽ RUB"))
        coins.append(Coins(coinName: "EUR", coinViewName: "€ EUR"))
    ...
    @IBAction func AddCoinItem(_ sender: UIBarButtonItem) {
        ...
        let action = UIAlertAction(title: "Set", style: .default) { (action) in
            ...
            if coinTextField.text! != "" {
                ...
                let viewCoinName: String
                
                if let coinSymbol = getSymbol(forCurrencyCode: coinTextField.text!) {
                    viewCoinName = "\(coinSymbol) \(coinTextField.text!)"
                } else {
                    viewCoinName = coinTextField.text!
                }
                
                let coinCustom = Coins(coinName: coinTextField.text!, coinViewName: viewCoinName)
            ...
            if cryptoCoinTextField.text! != "" {
                ...
                viewCoinName = "\(getCrypoCoinSymbol(cryptoCoinTextField.text!)) \(cryptoCoinTextField.text!)"
                
                let cryptoCoinCustom = Coins(coinName: cryptoCoinTextField.text!, coinViewName: viewCoinName)
